  ...
  def recommendTForS(rank:Int,
    ...
    val ratings = srcBlocks.cartesian(dstBlocks).flatMap {
        case ((srcIds, srcFactors), (dstIds, dstFactors)) =>
          ...
          output.toSeq //.filter(_._2._2 >= confidence)
    }
//      .mapPartitions(topKOfPartition(_, num))
  ...
  def recommendProductsForUsers(model:MatrixFactorizationModel,
    ...
    val rank = model.rank
    val srcFeatures = model.userFeatures.cache()
    val dstFeatures = model.productFeatures.cache()
    ...
    // TODO tuning use min-heap to pruning the top k (largest) elements for each key in each partition
    // need more test.
//      .mapPartitions(topKOfPartition(_, num))
  ...
  def topKOfPartition(iter: Iterator[(Int, (Int, Double))],
                      num: Int)
  :Iterator[(Int, (Int, Double))] = {
    iter.toSeq.groupBy(_._1)
      .mapValues(rs => rs.map(_._2).aggregate(new BPQ(num)(Ordering.by(x => x._1)))(seqOP, combOP))
      .mapValues(_.toArray)
      .flatMap{
